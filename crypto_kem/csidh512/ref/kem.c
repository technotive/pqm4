...
int crypto_kem_keypair(unsigned char *pk, unsigned char *sk){
  ...
  csidh((public_key*) pk, &base, (private_key*) sk);
  return 0;
}
...
int crypto_kem_enc(unsigned char *ct, unsigned char *ss, const unsigned char *pk){
  private_key skb;
  csidh_private(&skb);
  csidh((public_key*) ct, &base, &skb);
  csidh((public_key*) ss, (public_key*) pk, &skb);
  return 0;
}

/* ct = pkb, sk = ska; Recreate ss = H(ska*pkb) <= skb*pka=ska*pkb */
int crypto_kem_dec(unsigned char *ss, const unsigned char *ct, const unsigned char *sk){
  csidh((public_key*) ss, (public_key*) ct, (private_key*) sk);
  return 0;
}
